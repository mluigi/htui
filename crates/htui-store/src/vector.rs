//! Provides integration with the Qdrant vector database.
...
use qdrant_client::qdrant::{
    ...
    PointStruct, SearchPointsBuilder, SparseIndices, SparseVectorParamsBuilder,
    SparseVectorsConfigBuilder, UpsertPointsBuilder, Value as QdrantValue, Vector,
    VectorParamsBuilder, Vectors, VectorsConfigBuilder,
};
...
impl QdrantStore {
    ...
    pub async fn new(collection_name: &str) -> Result<Self, StoreError> {
        ...
        let client = builder
            .build()
            .map_err(|e| StoreError::Backend(format!("Qdrant client error: {}", e)))?;
        let embedder =
            Embedder::new().map_err(|e| StoreError::Backend(format!("Fastembed error: {}", e)))?;
    ...
    async fn init_collection(&self) -> Result<(), StoreError> {
        ...
        if !exists {
            ...
            vectors_config
                .add_named_vector_params("dense", VectorParamsBuilder::new(384, Distance::Cosine));
    ...
    async fn upsert(
        ...
        let dense_embeddings = self.embedder.embed_dense(vec![text_owned.clone()]).await?;
        let sparse_embeddings = self.embedder.embed_sparse(vec![text_owned]).await?;
        ...
        payload.insert("id".to_string(), QdrantValue::from(id.to_string()));
        ...
        let point = PointStruct::new(point_id, Vectors::from(vectors_map), payload);
...
impl VectorStore for QdrantStore {
    ...
    async fn search_concepts(&self, query: &str, limit: u64) -> Result<Vec<String>, StoreError> {
        ...
        let dense_embeddings = self.embedder.embed_dense(vec![query_owned.clone()]).await?;
        let _sparse_embeddings = self.embedder.embed_sparse(vec![query_owned]).await?;
