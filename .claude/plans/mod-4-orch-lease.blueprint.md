...
## 23. Milestone 5 follow-up (`mod-4-m5-fixups`, 2026-09-23)

A repair round on a branch cut from `main` at `a0da87e`, before milestone 6. It closes R-33, R-34 and R-35 (§22.4). Milestone 5 stays merged and closed; nothing here re-routes it. Each fix is test-first, and every red commit compiles and passes `clippy -D warnings`. Pins stay: store `CASES` 53, orch `CASES` 52, `.sqlx` 227.

### 23.1 Decisions (D146–D152)

| # | Closes | Decision |
|---|---|---|
| **D146** | R-33 (a) | `git::reconcile_parent` ties the merge to the primary checkout, git-only (no store column, so no migration). It takes the checkout path, the row's `before` and `after`, and the step. It answers `Some(first)` only when all four hold: `after` has exactly two parents `[first, second]`; its subject is D147's; `is_ancestor(checkout, before, first)`; and `merge_of(checkout, HEAD, before, second) == Some(after)`, `HEAD` being the checkout's `HEAD` read at the call. The first two are cheap and checked first. An agent's spoofed two-parent `htui: reconcile <step>` commit is never on the primary's first-parent line (htui's `merge --no-ff` puts the agent's tip on the second-parent side), so `merge_of` answers `None` or another merge, and the diff stays `before..after`. The check always runs against the checkout, never a `copy` tree: the merge exists only in the checkout, and a spoof in a copy tree is absent there. `merge_of` hides `before` (D142), so the walk stops at the step's base. |
| **D147** | R-33 (b) | The message test compares the **subject** only: the raw message up to its first `\n`, ASCII-trimmed, equal to `reconcile_message(step)`. A body a `merge.log` shortlog or a commit-msg hook appends no longer hides the merge. A hook that rewrites the subject still does; the diff then falls back to `before..after` (safe: a superset, never a lost commit). |
| **D148** | R-33 (c) | `Cli::merge_no_ff` runs `git -c merge.log=false merge --no-ff --no-edit -m <message> <after>`, so a primary with `merge.log=true` gets the exact message D25 names. `--no-verify` is **not** passed: the user's hooks stay their policy, and D147 covers the body they add. |
| **D149** | R-34 | Every command's window between its lease take and its `walk_leased` runs inside one engine helper that calls `release_lease` once on any `Err` it returns. A failed release already enters `DeadWalks` (D140), so the run is adoptable by this process's next sweep either way. The windows are `answer_guarded`, `retry_guarded`, `retry_group_guarded`, `select_fanout` and `resume` (its reads, the live-graph resolve, and both notes). `claim` has no window. Inside the helper the per-site releases go: `stale_first_write`'s release and `retry_group_guarded`'s `StaleWrite` release are folded into the helper's, so each `Err` releases exactly once. |
| **D150** | R-34 | `resume`'s topology-mismatch branch releases the lease it took whatever the run's status, because nothing walks the run. A `running` run is then adopted again by the next sweep of any process, this one included (D139). Consequence carried as R-36. |
| **D151** | R-35 | `Engine::release_lease` drops the run from `DeadWalks` on `StoreError::NotFound { entity: "run", .. }` (the row is gone, so no lease is left to give back), with a `warn`. Every other `Err` still enters or keeps it in the set. |
| **D152** | R-35 | "Row gone" and "store unreachable" are told apart by a failure hook on `MemStore`, not by the ghost-run convention. `MemStore` gains a small test seam: a `pub enum MemFault` (`RefreshLease`, `ReleaseLease`, `ItemTransition`) and a switch that makes the named write answer `StoreError::Unreachable` before it touches any state, until switched off. The W1 tests (`an_expired_walk_is_adopted_by_its_own_process_once_the_store_is_back`, `a_failed_release_is_retried_by_the_sweep`) and the D122 test use it for their outage instead of a not-yet-created run. `ItemTransition` is D149's test's forced failure of `unpark`'s item `transition`. No trait method, conformance case, store pin or `.sqlx` file changes. |

### 23.2 Tests (first failing test per decision)

- **D146** (real git, `gix_isolator.rs`): an agent branch ending in a spoofed two-parent `htui: reconcile <step>` commit still diffs `before..after`, so the agent's earlier commits stay in the diff.
- **D147/D148** (real git): a primary with `merge.log=true` in its repository config still diffs `after^1..after` after a moved-primary merge, and the merge commit's message is exactly `htui: reconcile <step>`.
- **D149** (engine unit): `unpark`'s item transition forced to fail (`MemFault::ItemTransition`) inside a gate answer; the command raises, and a second `sweep()` from the same process adopts the run.
- **D150** (engine unit): after a topology-mismatch `resume` of a `running` run, a second `sweep()` from the same process adopts it.
- **D151** (engine unit): a `DeadWalks` run whose release answers `NotFound { entity: "run" }` leaves the set; one whose release answers `Unreachable` stays.
