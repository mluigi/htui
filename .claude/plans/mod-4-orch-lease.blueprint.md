...
| `an_interrupted_step_out_of_budget_parks` (F-C) | `repoint` `retry_limit = 0` (`conformance.rs:455`); stall without the document | sweep → `Parked(Rest { AwaitingApproval, failure: Some(Interrupted { reset: true, .. }) })`; run and item are `awaiting_approval`; N3; `RetryStep` → `Err(RetryExhausted { .. })` (or `Retried` under A-8); `CancelRun` → `Cancelled` and the item is `open` |
