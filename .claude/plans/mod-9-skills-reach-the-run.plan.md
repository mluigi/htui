...
**Status: PROPOSED (2026-09-26), awaiting the maintainer's CONFIRM.**
...
- [ ] **OQ-8 — Judge steps carry no skills.** The PRD's D4 says "phase and judge steps", but
      `Placeholder::allowed_in` does not allow `{{skills}}` in a judge or handoff body
      (`prompt/template.rs:187-213`). `parse` refuses it there, so a judge prompt can never render
      a skill. **Default (D44):** the judge spec keeps `skills: Vec::new()` and its comment is
      corrected to cite the placeholder contract. `promote::handoff_spec` stops inheriting the
      phase's skills (`..phase`, `promote.rs:79-103`), because those skills would be scrubbed for
      nothing and could fail the handoff on a skill body. **Alternative:** resolve them for the
      judged phase anyway and record them as `not_placed`. That costs one read per judge for a
      record that only ever says "not rendered".
...
judge spec stays empty with a corrected comment, and the handoff spec stops inheriting skills.
...
- The PRD's gate decisions, this plan's D37–D47 and the maintainer's OQ answers win over prose.
...
- **Action**: D38, D39, D41. Regenerate `.sqlx` against a scratch database migrated to `0007`.
...
- **Tests first** (`engine.rs` tests over `FakeGraphSource` + `MemStore::demo()`):
  ...
  - `the_judge_and_the_handoff_carry_no_skills`: the recorded trim record's `skill_choices` is empty for both.
...
- Judge and handoff prompts are unchanged.
...
- **PRD D4** says judge steps carry skills; the placeholder contract makes that inert (OQ-8).
