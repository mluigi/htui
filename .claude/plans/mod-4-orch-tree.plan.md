...
**Complexity**: Large (a git library enters the workspace; four isolation modes over it; one
working-tree-update primitive that `gix` does not ship and this plan has to compose; a process
runner; two seam writers with their conformance leg; three of ANA-2's validation criteria, all of
which need a real repository on disk).
...
analysis and the library disagree, or where the library's surface could not be verified. **OQ-1 is
the one that changes the whole shape of T2 and T5 if answered the other way.**

- [ ] **OQ-1 — `gix` 0.87.1 has no `git worktree add`, `remove` or `prune`, and no
      ...
      **Default adopted (D23, D25):** stay pure `gix` — build the linked-worktree layout from
      primitives, check out with the same `State::from_tree` + `gix_worktree_state::checkout` +
      `File::write` sequence `gix`'s own clone uses, and compose the working-tree update for
      reconciliation from a tree diff plus that checkout. Cost: ~350 lines in `isolate/git.rs`
      that re-implement four `git` verbs, and one API (D25's restricted index) this plan could not
      verify. **Alternative:** shell out to `git` for exactly `worktree add --lock`, `worktree
      remove`, `worktree prune` and `merge --no-ff` behind the same `git.rs` seam, keeping `gix`
      for every read and every ref write. That contradicts ANA-2 `:1777` and would need the
      analysis amended; it is roughly a third of the code and inherits `git`'s own behaviour on
      submodules, filters and `autocrlf`, which MOD-16 would otherwise have to re-verify for the
      hand-built path (PRD `:296`). The plan is written for the default; the alternative swaps
      `git.rs` and nothing above it.
...
The single largest risk is not git semantics but **library surface**: `gix` 0.87.1 does not
implement `git worktree add` (OQ-1). The plan is honest about what it composes from primitives, what
it verified by reading the crate source, what it verified by compile probe, and what it could not
verify at all (the last section).

## Design decisions (settled here, not in code review)

| # | Decision | Why |
|---|---|---|
| D22 | **`gix = "0.87.1"`, `default-features = false`, features `["sha1", "max-performance-safe", "parallel", "index", "revision", "blob-diff", "dirwalk", "status", "merge", "worktree-mutation"]`, declared once in `[workspace.dependencies]` and consumed only by `htui-orch`.** Every `gix` call runs under `tokio::task::spawn_blocking` inside a sync function in `isolate/git.rs` that takes owned `PathBuf`s and returns owned values; no `gix::Repository` is ever held across an `.await`. | 0.87.1 is the current crates.io release (`cargo search gix`, 2026-09-22; `rust-version: 1.85`, under the workspace's 1.98 pin, `rust-toolchain.toml:2`). The feature list is read from the crate's manifest, not recalled: `sha1` is **required** and is a default feature that vanishes under `default-features = false` (`gix-0.87.1/Cargo.toml:132-139`, `:228-233`) — the first probe without it failed inside `gix-hash` with `E0004: type &Kind is non-empty` on an empty enum; `status` gates `is_dirty` (`Cargo.toml:241-245`, `src/status/mod.rs:156-168`); `merge` gates `merge_commits` (`:184-188`); `worktree-mutation` gates the `gix_worktree_state` re-export (`:259-262`, `src/worktree/mod.rs:8`); `parallel` is what makes `Repository: Send` at all (`src/types.rs:155-158`). `Repository` is `Send` and **not `Sync`** (`types.rs:148`), and `IsolatorFuture` is `Send` by its alias (`crates/htui-orch/src/isolate.rs:32`), so the only shape that compiles and stays honest is blocking work handed owned inputs. `htui-core` and `htui-store` gain no `gix` edge (ANA-2 §8's dependency-weight argument, `docs/ANA-2.md:1646-1652`). |
| D23 | **The `worktree` mode builds the linked worktree from primitives, because `gix` has no `worktree add`.** In order: (1) `Repository::reference("refs/heads/htui/<step_id>", before_hash, PreviousValue::MustNotExist, log)` creates the branch (`gix-0.87.1/src/repository/reference.rs:79-87`); (2) `git.rs` writes `<git_dir>/worktrees/htui-<step_id>/{HEAD, commondir, gitdir, locked}` and the `<path>/.git` gitdir file — the layout `git worktree list` and `Repository::worktrees()` (`src/repository/worktree.rs:46`) both read; (3) `gix::index::State::from_tree(&tree, &objects, …)` → `gix::index::File::from_state(state, <worktrees dir>/index)` → `gix::worktree::state::checkout(&mut index, path, objects.into_arc()?, &files, &bytes, &interrupt, opts)` with `destination_is_initially_empty = true` → `index.write(Default::default())`, which is verbatim the sequence `gix`'s own clone runs (`src/clone/checkout.rs:113-146`). `locked` carries `htui run <run_id>` (the `--lock` of ANA-2 `:914`). Remove = delete `<path>` and the `worktrees/htui-<step_id>` entry; prune = delete every `worktrees/htui-*` entry whose `gitdir` points at a missing path, **never** an entry without the `htui-` prefix. | OQ-1 states the absence and the alternative. Building the layout by hand is what makes ANA-2's own oracle for criterion 11 — "`git worktree list` in the managed repo shows them" (`docs/ANA-2.md:2117`) — true without a `git` subprocess in production. Scoping prune to `htui-*` entries is `R-ID-4` applied to metadata: a user's own linked worktree is not `htui`'s to prune. |
...
| D25 | **Reconciliation of one winner is: refuse unless the primary tree is clean *and* still at the step's `before_hash`; merge with `merge_commits`; refuse on any unresolved conflict; write the merged tree; create a two-parent commit on the primary branch with `commit_as` under an `htui` signature; update the primary working tree by diffing old tree → merged tree and checking out only the changed paths; write the index; record the merge commit as the winner's `after_hash`.** Where in the walk: one engine helper, `reconcile_done_step`, called after a step reaches `done` — `gate::apply`'s `(OnFailure \| Never, Ok)` arm (`crates/htui-orch/src/gate.rs:383-388`) and `answer_gate`'s `Approved \| Skipped` (`crates/htui-orch/src/engine.rs:376-377`) — and before `Landing::Advance` / `run_to_rest` continues. A refusal parks the **run** (step stays `done`) through a new `park_run(reason)` in `engine.rs`: run `running → awaiting_approval`, item `in_progress → awaiting_approval`, `item_note` naming the path list — the blueprint's H-10 three-write order, with R-5 still carried. `local` is the identity; `shared_serialized` is the identity when the tree's `HEAD` equals the winner's `after_hash` (the only milestone-3 case; D26); `copy` first walks `before..after` from the copy's odb into the primary's (OQ-5). | The steps are ANA-2 `:975-988` in order; `:978` names `dirty_primary_tree`, `:983` `merge_conflict` with the path list, `:987-988` the `after_hash` update. "Still at `before_hash`" is this plan's addition: `:948-953` branches every phase from the primary's *current* HEAD so later phases see earlier commits, and a primary that moved under a step is a three-way merge whose working-tree update no longer has a known base; parking with `primary_moved: <hash>` is the honest refusal. The API is read from source: `merge_commits` (`src/repository/merge.rs:181-188`) returns `merge::commit::Outcome { tree_merge: tree::Outcome { tree: Editor, conflicts, … } }` (`src/merge.rs:26-38`, `:116-128`) with `has_unresolved_conflicts(TreatAsUnresolved)` (`:135`) and `Conflict` rows that name paths; `tree_merge_options()` (`src/repository/merge.rs:84`); `commit_as` (`src/repository/object.rs:361-370`) rather than `commit` (`:461-467`) because the latter reads the committer from config and a managed repo may carry none; `diff_tree_to_tree` (`src/repository/diff.rs:50`). The "only the changed paths" checkout is `State::from_tree(merged)` → `remove_entries` keeping the paths `diff_tree_to_tree` reported → `checkout` with `overwrite_existing = true` — every call compile-verified (fact ledger below); what the probe cannot prove is its behaviour on a populated directory, which T2's `merge_no_ff_…` test pins. The fallback, if that test fails, is a full-index checkout, which rewrites every tracked file's mtime and makes the primary tree's build cache cold once per reconcile. Always `--no-ff` even when fast-forwardable, per `:980`. |
...
| D38 | **`GixIsolator::prepare` is idempotent for `worktree` and `copy` even though the trait promises nothing and the engine calls it once.** If `<root>/<run>/<step>/<repo>` exists with a `.git` entry, `prepare` reuses it and reports `before_hash` = the target of `refs/heads/htui/<step_id>` (not the current `HEAD`). `shared_serialized` and `local` are idempotent by construction. The trait doc gains one sentence saying so; the fake stays non-idempotent (it mints fresh hashes, `fake.rs:98-109`). | The engine threads `cwd` down one call frame precisely because a second `prepare` "is a second set of trees and a second `before_hash`" (`engine.rs:1069-1073`), and `a_walk_prepares_each_step_exactly_once` pins one call (`:2550-2588`). What that does not cover is a crash *between* `prepare` and `upsert_step_tree` (`:791-801`): milestone 5's sweep re-derives from rows (M2 D16; invariant 9, `docs/ANA-2.md:139-142`), finds no tree row, and re-runs the step — whose second `prepare` would then hit `PreviousValue::MustNotExist` on an existing branch and strand the step. Idempotence costs one `exists()` and one ref read and removes a whole class of unrecoverable states. |
| D39 | **Every git write is wrapped in `git::with_retry`: three retries at 200, 400 and 800 ms when the error is classified as a lock error.** Classification: an `io::ErrorKind::AlreadyExists` from `gix::lock`, or an error whose `Display` contains `.lock` or `cannot lock`. Reads are not retried. | ANA-2 `:935-942` and PRD `:389`. `gix::lock` is re-exported (`gix-0.87.1/src/lib.rs:141`) and its `acquire::Fail::AfterDurationWithBackoff(Duration)` is a *quadratic* backoff (docs.rs, gix-lock 24.0.0), not the 200/400/800 the analysis fixes — so the schedule is ours and sits above the library. The message-based half of the classifier is a weak point and is listed under Risks; it exists because `gix`'s error enums are per-operation and this plan could not enumerate every lock-bearing variant from the tree. |
| D40 | **Tests build real repositories with `gix::init` + `commit_as` under `tempfile::TempDir` and need no `git` binary; the one `git worktree list` oracle (criterion 11) runs only when `git` is on `PATH` and prints `skipped: git not on PATH` otherwise.** No Postgres anywhere in the isolator tests; the `command_run` writer's Postgres twin is the only Postgres-gated test and sits behind `HTUI_TEST_DATABASE_URL` like every other. | `gix::init` (`src/lib.rs:332`) and `commit_as` (`object.rs:361`) are enough to mint a repository with history. `tempfile = "3"` is a dev-dependency in every other crate (`crates/htui-core/Cargo.toml:31`, `htui-agent:70`, `htui-store:45`, `htui:64`). The skip sentence is the README's own convention for a missing server (`README.md:477-480`), so the workspace gate stays green on a box with neither. |
...
|---|---|---|---|---|
| **Tree** | `<root>/<run>/<step>/<name>/`, branch `htui/<step_id>` at `before_hash`, checked out by D23's sequence, `locked` | `<root>/<run>/<step>/<name>/`, a filesystem copy including `.git/` minus `copy_exclude`, reset to `HEAD` if the source was dirty (D35) | `repo_box_path.local_path` itself; the per-`(box, repo)` mutex held from `prepare` to `cleanup` (D43) | `repo_box_path.local_path` itself, nothing held |
...
| **Cleanup (run terminal)** | delete the directory and the `worktrees/htui-<step>` entry, then prune `htui-*` entries (D23) | delete the directory | release the mutex | nothing |
| **Refused when** | the repo has submodules (`submodules()` yields any, `src/repository/submodule.rs:93`) → `submodules: worktree isolation is not supported`; `<name>` collides between two repos of one project → `duplicate repo name` | no `.git` directory → `not a git checkout`; `.git` is a file → `source is a linked worktree`; measured size × copies > cap → `copy would need N bytes; cap is M` | never in this milestone (D29) | never in this milestone (`fan_out > 1` is already refused at snapshot time, `crates/htui-orch/src/graph.rs:425-429`; rule L is milestone 5's, D44) |
...
| Skip sentence on a missing external | `skipped: HTUI_TEST_DATABASE_URL not set` | `README.md:477-480` |
...
| `Cargo.toml` | edit | T2 | `[workspace.dependencies]` gains `gix` (D22); `walkdir`, `process-wrap`, `windows`, `dirs` are already there (`:51`, `:59-64`, `:101`) |
| `crates/htui-orch/Cargo.toml` | edit | T2 | `gix`, `walkdir`, `process-wrap`, `windows` (cfg windows), `dirs`; dev `tempfile = "3"`; `tokio` features `["sync", "rt", "process", "io-util", "time", "fs"]` |
...
| `crates/htui-orch/src/isolate/git.rs` | create | T2 | the sync `gix` wrapper: open, head, dirty, branch, worktree layout, checkout, merge, odb range copy, retry (D22, D23, D25, D39) |
...
| `crates/htui-orch/tests/gix_isolator.rs` | create | T6 | criteria 11, 12, 13; the per-mode table; reconcile; idempotent prepare; the `git worktree list` oracle (D40, D41) |
...
constant; **no second migration**; **no `git` subprocess in `src/`** (OQ-1's default); commit
incrementally, because uncommitted work does not survive the session.
...
### Task 2: `htui-orch` — the manifest and `isolate/git.rs`
- **Test first**: in `git.rs`'s `#[cfg(test)]`: `init_commit_and_head_round_trip` (a repo made
  with `gix::init` + `commit_as` reports `head()` equal to the commit it made);
  `is_dirty_ignores_untracked_and_sees_a_modified_tracked_file` (D24);
  `a_linked_worktree_is_listed_by_gix_and_by_git` (D23: after `add_worktree`, `Repository::
  worktrees()` on the main repo lists one proxy whose `id()` is `htui-<step>` and `is_locked()`;
  when `git` is on `PATH`, `git worktree list --porcelain` names the path; else the skip sentence);
  `remove_and_prune_touch_only_htui_entries`; `merge_no_ff_of_a_descendant_makes_a_two_parent_commit`;
  `a_conflicting_merge_is_refused_with_the_path`; `copy_range_moves_every_object_between_odbs`
  (OQ-5); `with_retry_retries_three_times_on_a_lock_error_and_not_on_others` (D39). Every test
  builds its repository under `tempfile::TempDir`.
- **Action**: the workspace and crate manifest lines (D22) in the **first** commit, so the crate
  ...
  `Vec::new()`; the idempotence sentence on the trait; then `git.rs` as sync functions over
  `&Path` returning owned values: `open`, `head`, `is_dirty`, `create_branch`, `branch_target`,
  `add_worktree` (layout + checkout), `remove_worktree`, `prune_htui_worktrees`,
  `checkout_tree_over` (D25's working-tree update — implement the restricted-index form if the
  API exists, else the full form, and **say which in the doc comment**), `merge_no_ff`,
  `copy_range`, `with_retry`. Each public function documents the `gix` call it wraps with the
  `gix-0.87.1/src/...` line this plan cites, so a fact-checker can re-verify without recall.
- **Mirror**: `launch.rs:1110-1160` for the blocking discipline; `isolate.rs:40-52` for errors.
- **Validate**: `cargo test -p htui-orch --all-features` then `cargo clippy -p htui-orch
  ...
  once, to confirm the feature set is the one D22 names and nothing enabled `blocking-network-client`.
...
- **Test first** (each over a `TempDir` with one or two `gix::init` repos and a config map, D34):
  ...
  `a_scratch_root_inside_a_repo_is_refused` (invariant 4); `cleanup_removes_every_tree_and_leaves_
  the_repo_untouched` (criterion 13's isolator half: `git worktree list` oracle when available,
  `Repository::worktrees()` always, the managed tree's `HEAD` and dirtiness unchanged, no
  `refs/heads/htui/*` checked out anywhere — `head()` of every repo is compared);
...
- **Test first**: in `conformance.rs` over the fake (`CASES` 15 → 18):
  ...
  out. The `git worktree list` oracle applies where `git` exists.
...
(`README.md:477-480`); without `git` on `PATH` the three oracle assertions print `skipped: git not
on PATH` and pass (D40); everything else in this milestone runs on a box with neither.

## Risks

| Risk | Likelihood | Mitigation |
|---|---|---|
| **`gix` has no `worktree add`, and the hand-built layout diverges from `git`'s** — a field `git` writes that this plan omits, or an `extensions.worktreeConfig` the repo has set | High | OQ-1 names the choice; T2's test compares the layout against `git worktree list --porcelain` where `git` exists and against `Repository::worktrees()` always; `worktreeConfig` repos are refused with a sentence until MOD-16 verifies them. If the maintainer takes the alternative, only `git.rs` changes |
| The restricted-index checkout of D25 compiles but misbehaves on a populated directory (a removed path left behind, a filter not applied), and the fallback rewrites every tracked file on reconcile, making the primary tree's Cargo fingerprints cold once per phase | Medium | T2's `merge_no_ff_…` test asserts the working tree after reconcile file by file, including a deletion; the fallback is correct, only slow; the doc comment on `checkout_tree_over` states which form shipped |
...
| The lock classifier of D39 matches on message text and misses a lock error, so a contended write fails without retrying | Medium | Named in D39 as a weak point; the retry wrapper logs the classified and unclassified error kinds at `warn`, so the first miss in the field is a one-line fix; PRD `:389`'s eight-of-thirteen figure is for parallel siblings, which are milestone 4's |
...
- [ ] `gix` 0.87.1 is in `Cargo.lock` with exactly D22's feature set (`cargo tree -p htui-orch -i
      gix --edges features`), consumed by `htui-orch` only; `htui-core`'s and `htui-store`'s
      manifests show a zero diff.
...
- [ ] ANA-2 §12 criteria 11, 12 (the record half) and 13 pass end to end against `GixIsolator` +
      ...
      oracle runs when `git` is on `PATH` and prints `skipped: git not on PATH` otherwise.
...
- [ ] `cargo test --workspace --all-features -- --test-threads=1` green with Postgres up; green
      minus the `skipped:` lines with Postgres down; green on a box without `git`.
...
- [ ] No migration, no `.snap` re-recorded, no `git` subprocess under `crates/htui-orch/src/`
      (grep `Command::new("git")` is empty), no overlap or admission logic anywhere in this
      milestone's files (D44).
...
| Workspace `tokio` already has `process`, `io-util`, `fs`; `process-wrap`, `windows`, `walkdir`, `dirs` are workspace deps; `similar` is declared at `:65` but is **not** idle — `htui-agent` consumes it | `Cargo.toml:32-33`, `:51`, `:59-64`, `:65`, `:101`; `crates/htui-agent/Cargo.toml:35`, `crates/htui-agent/src/acp/fs.rs:106-118` |
...
| `git` 2.43.0 is on this box at `/usr/bin/git` (test oracle only) | `which git` |
...
2. **`edit_reference` with `Target::Symbolic` for `HEAD` compiles** as OQ-3's alternative would
   ...
   `HEAD` file directly, so the default path does not depend on it.

Two things the probe does **not** prove and T2's tests must: that the hand-written worktree
layout of D23 is one `git worktree list` accepts (a compile cannot check file contents), and that
`checkout` over a subset index behaves as D25 expects on an already-populated directory
(`overwrite_existing = true`, `destination_is_initially_empty = false`).
...
|---|---|---|
| `gix = "0.87.1"`, `default-features = false`, D22's feature list resolves and compiles | confirmed | fresh probe crate green; `cargo tree -e features -i gix` adds nothing beyond D22's closure |
...
| Every symbol D23/D25/D35 intend to call exists with the claimed shape | confirmed | all compiled under D22's feature set: `gix::init`, `Repository::reference`, `State::from_tree`, `gix_worktree_state::checkout` + its `Options` fields, `index::File::write`, `is_dirty()`, `commit_as` without `command`, `merge_commits`, `Conflict.ours.location()`, `rev_walk(..).with_hidden(..)`, `find_object`/`write_object`, `State::remove_entries` |
...
| `Conflict.ours` is `gix::diff::tree_with_rewrites::ChangeDetached` | confirmed, naming imprecise | same underlying type, re-exported as `gix::object::tree::diff::ChangeDetached` (`object/tree/diff/mod.rs:11`); `.location() -> &BStr` at `gix-diff-0.67.1/src/tree_with_rewrites/change.rs:541` |
...
| D40: `tempfile` is available and no test needs a `git` binary | partial | `tempfile = "3"` is a **per-crate** dev-dependency (`htui-core/Cargo.toml:31`, `htui-store:45`, `htui:64`, `htui-agent:70`), **not** a `[workspace.dependencies]` entry, and `htui-orch` has none — T2 adds it the same way the other crates did. No test spawns a `git` process today |
...
| D38's premise: a crash between `prepare` and `upsert_step_tree` leaves no row | confirmed, not conflated | `upsert_step_tree` has no precondition on either store (`mem.rs:3617-3627` plain insert; `pg/write.rs:3187-3223` `ON CONFLICT … DO UPDATE`); `PreviousValue::MustNotExist` is the `gix`-ref concept D23's branch creation uses (`gix-ref-0.67.1/src/transaction/mod.rs:50`), which is what D38 actually attributes it to |
