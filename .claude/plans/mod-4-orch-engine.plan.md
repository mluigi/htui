...
| D10 | **The front-matter verdict parser lives in `gate.rs`, and only an exact `request-changes` rejects.** Vocabulary: `approve` and `request-changes`. A review document whose front matter is absent, unparseable, or carries any other value is **not** a rejection; the unexpected value is recorded verbatim in `gate_note` so the operator sees it. | ANA-5 §12 criterion 16's parser is MOD-4's by `HANDOFF.md:208`; ANA-2 defines settle `rejected` off "front matter" (`:440`) and names no parser, no site and no second legal value. Treating silence as rejection would burn `R-ORCH-3`'s retry budget on a parse bug — the expensive direction of a coin flip the document does not call. The gate is where the settle outcome is computed, so the parser has no other honest home. |
