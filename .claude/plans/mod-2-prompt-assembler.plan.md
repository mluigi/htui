...
| **D109** | **The preview refuses on `Backend::Offline` with one named sentence (`PROMPT_ON_SERVER_ONLY`), and this is now the product's direction rather than a milestone-9 expedient.** Maintainer decision 2026-09-11, answering the blueprint's E-2: `htui` is an **online-only** program. A box that cannot reach its Postgres shows a warning that the database is unreachable; it does not carry a second, writable life of its own. The wider reversal — withdrawing ANA-10's "a box with no DSN is a complete box", the `R-STO-*` amendments and the disposition of MOD-17/18/19 — is **not** milestone 9's work and is minted as its own item. | The blueprint found that an offline preview is not one missing setting but four unmirrored tables (`prompt_template`, `skill`, `skill_version`, `skill_binding`, `box_tool`), so "preview offline" meant a cache migration that renumbers MOD-4's a second time and rebuilds every box's mirror. Weighed against a feature the maintainer does not want, that is cost for nothing. Refusing names the real reason in the one place a user meets it. |
| **D110** | **No `Repo` fixture rows and no `repos()` read in this milestone. `trim_record.excerpts.roots` is `[]` in the preview and ANA-5 §12 criterion 12 is proved by unit test over `FakeRepoReader`, not from the running binary.** Maintainer decision 2026-09-11, answering the blueprint's E-4. **Stated plainly because it is the one place this milestone's evidence is weaker than the PRD asked for**: the preview was justified partly as the thing that exercises the assembler end to end, and the excerpt audit is the corner it does not reach. | The smaller scope, and cheap to reverse: adding the rows later is additive to `fixtures.rs` and `pg/demo.rs` and changes no signature. Worth recording that the question was **not** an offline one — `repo` rows exist on an online box too — so if the empty root list reads badly in use, it is ~40 lines and not a design change. |
| **D111** | **§4.2's `command_queue` section ships with the blueprint's two sentences, verbatim** (B.9's `COMMAND_QUEUE_TEXT`). Maintainer approval 2026-09-11 under ANA-5 Open-for-the-maintainer 4, which makes the bodies theirs. | ANA-5 fixes that the section exists and never writes its words, and every byte of it is hashed into `prompt_digest` — so settling it before T64 accepts a golden snapshot costs nothing, and settling it after invalidates every one of them. |
