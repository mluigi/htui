...
- [ ] **OQ-4 — Does the store re-run `parse`?** **Default (D4):** yes — `append_prompt_template`
      refuses what `parse` refuses with `StoreError::Constraint`, so no caller (a future CLI, MOD-55)
...
| D15 | **What milestone 1 leaves alone, by name.** Milestone 2's engine and preview wiring (`engine.rs:4310`, `:4925-4927`, `preview.rs:227`); every skill writer and the Skills view's content (milestone 3, ANA-22); `SKILL.md` import; MOD-55's agent help; syntax highlighting, bracketed paste, undo; any `app_setting` for the editor command; any migration; `htui-orch`; `Backend`'s read methods; `htui_core::prompt::template` (read-only use). `docs/**`, `HANDOFF.md` and the PRD are the main thread's. | PRD scope and D6 sequencing. |
