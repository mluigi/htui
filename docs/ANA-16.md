...
| Container images and digests, child-box profiles | Not yet (MOD-44, §5.3) | - | New config either way |
...
7. **Live relay and permission relay.** Transient deltas and `permission_request` go worker to
   ...
   worker. This is MOD-42 with push instead of polling; it still needs `pump`'s parked-request fix
...
lease, adopt, step status, events, usage, finish), which is a constraint on MOD-41 now (§8).
...
| Permission relay | Rows plus poll or `NOTIFY` (MOD-42) | Push through server; same `pump` fix | Same |
| Live streaming | Flush bursts; `NOTIFY` deltas < 8000 B, lossy (MOD-46) | Native relay | Native relay |
...
3. **O5a's gains are real and are all about workers on boxes the user does not fully trust or
   ...
   host's worker (so a container never holds the DSN, §5.3 and MOD-44 in §8), those gains are small next to a new
...
(`R-USR-3`), more worker processes than the Postgres connection budget allows, or MOD-46's `NOTIFY`
...
IDs minted 2026-09-24 when the items were opened (placeholders G, A-F, H, I became MOD-37..45 in that order), renumbered 2026-09-25 to MOD-40..48 because main had meanwhile minted MOD-37 (from MOD-4) and MOD-38, MOD-39 (from ANA-11); see §10. Order is dependency order.

**Phase 1 - no server (O3 + O2).**

1. **MOD-40 - Multi-writer store hardening.** `R-ID-3`, `R-HIS-1`, ANA-2 inv. 1.
   ...
   - Depends on nothing; MOD-41 depends on it.
2. **MOD-41 - Headless worker (`htui worker`).** `R-ORCH-12`, `R-ID-2` (amended), `R-STO-1`,
   `R-NF-2`, `R-NF-3`.
   ...
   - Depends on MOD-40, MOD-4 (M6) and MOD-7.
3. **MOD-42 - Permission and control relay through Postgres.** `R-AGT-1`, `R-HIS-1`, `R-TUI-6`.
   ...
   - Depends on MOD-4 (M6). MOD-41 consumes it. Its row protocol is the payload O5a later pushes.
4. **MOD-43 - Remote dispatch in the TUI.** `R-ORCH-11`, `R-ORCH-12`, `R-TUI-1`, `R-NF-3`.
   ...
   - Depends on MOD-41 and MOD-42, and on MOD-12 for auto mode.
5. **MOD-44 - Container execution environment.** `R-BOX-1..3`, `R-AGT-5`, `R-AGT-6`, `R-AGT-9`,
   ...
   - Linux and macOS first; Windows deferred to MOD-16. Depends on MOD-7; needs MOD-41 to survive TUI
     exit.
6. **MOD-45 - Remote box provisioning over SSH.** `R-BOX-1`, `R-BOX-4`, `R-AGT-9`, `R-STO-1`.
   ...
   - Depends on MOD-41 and MOD-22.
7. **MOD-46 - Live streaming via `NOTIFY` (optional).** `R-HIS-1`, `R-NF-3`.
...
8. **MOD-47 - `htui server` control plane.** `R-NF-2`, `R-ID-2`, `R-ORCH-12`, `R-STO-1`, `R-STO-5`
   ...
   - A decision on server-down behaviour: reset-and-retry kept, or an `unknown`/`lost_after` state
     (amends ANA-2).
   - Depends on MOD-41 (narrow store surface), MOD-42, MOD-40.
9. **MOD-48 - Config manager and secret distribution.** `R-ID-3`, `R-AGT-9`, `R-AGT-10`, `R-SEC-1`,
   `R-SEC-2`.
   ...
   - Depends on MOD-47 and MOD-10 (secret provider).
...
1. **Headless DSN (`R-STO-1`).** The Linux keyring backend is `sync-secret-service` only
   ...
   `R-STO-1` forbids). Needs a maintainer amendment before MOD-41. Phase 2 does not remove the
...
3. **Environment inheritance.** `spawn_supervised` has no `env_clear` (`launch.rs:1114-1121`).
   ...
   `docker inspect` and `/proc`. Verify before MOD-44.
...
10. **Two executors on one box.** Per-process `lease_owner` keeps an in-process `run_worker` and a
    ...
    rather than safety.
11. **Stale lease holder (C1) and clock skew (C2).** Until MOD-40, a process resumed from suspend can
...
12. **Box identity by hostname (C4).** A container or cloned VM with a duplicate hostname silently
    ...
    hostname through is unverified. Fix before MOD-44 and MOD-45.
...
14. **Version skew (C5).** Until MOD-40, one box migrating locks out every older worker at its next
...
15. **Phase 2 costs.** A new daemon on the dispatch path (a second SPOF beside Postgres for workers),
    ...
    creeping toward a full `WriteStore` mirror (MOD-25's third store). The narrow surface in MOD-41 is
    the guard.
16. **Server-down semantics.** Reset-and-retry after 120 s versus an `unknown`/`lost_after` state
    that amends ANA-2's single liveness marker. Decide in MOD-47.
...
| 2026-09-24 | After the maintainer's challenge ("wouldn't a central server be better? what if multiple agents write at the same time on postgres?"): added §6.1 showing concurrent writers are already coordinated by CAS, row locks and `SKIP LOCKED`, and listing eight multi-writer gaps (C1-C8) that bind any design; corrected hazard 10 (admission is serialised by the box-row lock); added §6.2's config inventory; split O5 into O5a (control plane, TUI direct) and O5b (full server) and compared them with O3 on twelve axes. Verdict revised from "no server" to **phased**: O3 + O2 first, with MOD-40 (store hardening) and a narrow worker-store surface; O5a as phase 2 (MOD-47 server, MOD-48 config manager) on stated triggers; O5b rejected. Phase 2 requirement amendments added |
| 2026-09-25 | Renumbered the spawned items from MOD-37..45 to MOD-40..48, keeping their order (G, A-F, H, I). This analysis was closed on a branch while main minted MOD-37 (orchestrator hardening, from MOD-4) and MOD-38, MOD-39 (from ANA-11); IDs are never reused, so the ANA-16 items took the next free numbers. No content changed |
